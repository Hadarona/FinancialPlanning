...
describe("security hardening (Stage H / D-SEC-*)", () => {
  ...
  describe("CORS allowlist", () => {
    it("grants the configured origin credentials and denies a foreign origin any CORS headers", async () => {
    ...
    it("fails a foreign-origin preflight (no allow headers), so credentialed JSON requests never fire", async () => {
  ...
  describe("input limits (D-SEC-B3)", () => {
    it("rejects a body over the documented 32kb limit with the 413 envelope", async () => {
    ...
    it("rejects unparseable JSON with a 400 VALIDATION_ERROR, not a 500", async () => {
      const res = await fetch(`${ctx.baseUrl}/auth/register`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: '{"email": "broken"',
  ...
  describe("ownership matrix (D-SEC-B1): every private endpoint x anonymous + foreign session", () => {
    it(
      "rejects every private endpoint for anonymous (401) and foreign (404, no mutation) callers",
      async () => {
        const MONTH = "2026-07";
