...
describe("GET /insights/:month", () => {
  ...
  it("returns one coherent kit-fixture response: totals 842,000/918,000, shares sum 100 (D-INS-B1/B2/B3)", async () => {
  ...
  it("compares January with December of the previous year (D-INS-B4)", async () => {
    const { client, userId } = await registerUser(ctx.baseUrl);
  ...
  it("returns an explicit no-comparison state when the previous month has no budget (D-INS-B5)", async () => {
    const { client, userId } = await registerUser(ctx.baseUrl);
  ...
  it("never lets another user's transactions enter the aggregation (D-INS-B6)", async () => {
  ...
  it("aggregates ~1,000 transactions within the local performance budget (D-INS-B7)", async () => {
    const { client, userId } = await registerUser(ctx.baseUrl);
