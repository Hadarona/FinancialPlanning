...
describe("transactions API", () => {
  ...
  it("creates, recalculates, deletes, and rolls the aggregate back (D-EXP-B1, D-EXP-Q1 enabler)", async () => {
    ...
    const deleteRes = await client.request(`/budgets/2026-07/transactions/${transaction.id}`, {
      method: "DELETE",
    });
  ...
  it("sums minor units exactly: 1099 + 2101 = 3200, no float drift", async () => {
    const { client, userId } = await registerUser();
    await insertBudget(userId, "2026-07");
  ...
  it("rejects every invalid payload without mutating anything (D-EXP-B2)", async () => {
    const { client, userId } = await registerUser();
    await insertBudget(userId, "2026-07");

    const badPayloads = [
      // Unknown category.
      { payload: { categoryId: "yachts", amountMinor: 100, occurredOn: "2026-07-10" }, field: "categoryId" },
      // Date outside the month (previous, next, impossible day).
      { payload: { categoryId: "fun", amountMinor: 100, occurredOn: "2026-06-30" }, field: "occurredOn" },
      { payload: { categoryId: "fun", amountMinor: 100, occurredOn: "2026-08-01" }, field: "occurredOn" },
      { payload: { categoryId: "fun", amountMinor: 100, occurredOn: "2026-07-32" }, field: "occurredOn" },
      // Non-positive / non-integer / malformed amounts.
      { payload: { categoryId: "fun", amountMinor: 0, occurredOn: "2026-07-10" }, field: "amountMinor" },
      { payload: { categoryId: "fun", amountMinor: -500, occurredOn: "2026-07-10" }, field: "amountMinor" },
      { payload: { categoryId: "fun", amountMinor: 10.5, occurredOn: "2026-07-10" }, field: "amountMinor" },
      { payload: { categoryId: "fun", amountMinor: "100", occurredOn: "2026-07-10" }, field: "amountMinor" },
      // Oversized note.
      {
        payload: { categoryId: "fun", amountMinor: 100, occurredOn: "2026-07-10", note: "x".repeat(201) },
        field: "note",
      },
      // Unknown key (strict schema).
      { payload: { categoryId: "fun", amountMinor: 100, occurredOn: "2026-07-10", isAdmin: true }, field: null },
    ];
  ...
  it("makes retries idempotent via clientRequestId: one row, 201 then 200 (D-EXP-B6)", async () => {
    const { client, userId } = await registerUser();
    await insertBudget(userId, "2026-07");
  ...
  it("blocks cross-user add and delete: 404, nothing leaks or mutates (D-EXP-B3/B4)", async () => {
  ...
  it("returns the same 404 body for nonexistent and malformed transaction ids", async () => {
    const { client, userId } = await registerUser();
    await insertBudget(userId, "2026-07");

    const missing = await client.request(`/budgets/2026-07/transactions/${randomUUID()}`, {
      method: "DELETE",
    });
  ...
  it("paginates with enforced bounds and deterministic ordering", async () => {
    const { client, userId } = await registerUser();
    await insertBudget(userId, "2026-07");
    ...
    const pageOne = await client.request("/budgets/2026-07/transactions?limit=2&offset=0");
    ...
    const negativeOffset = await client.request("/budgets/2026-07/transactions?offset=-1");
    expect(negativeOffset.status).toBe(400);
  }, SLOW_TEST_TIMEOUT);

  it("never writes expense notes into the request/error logs (D-EXP-B5)", async () => {
    const { client, userId } = await registerUser();
    ...
    expect(
      requestEntries.some(
        (entry) => entry.method === "POST" && String(entry.route).includes("/transactions"),
      ),
    ).toBe(true);
  }, SLOW_TEST_TIMEOUT);
});
